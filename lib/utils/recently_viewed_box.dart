...
class RecentViewedBox {
  ...
  static void addToList(MaterialModel newMaterial) {
    if (recentlyViewed
        .any((element) => element.fileName == newMaterial.fileName)) {
      recentlyViewed
          .removeWhere((element) => element.fileName == newMaterial.fileName);
    }
    recentlyViewed = [newMaterial, ...recentlyViewed];

    recentBox.put(
        recentKey, recentlyViewed.map((e) => jsonEncode(e.toJson())).toList());
  }

  static void remove(MaterialModel material) {
    if (recentlyViewed
        .any((element) => element.fileName == material.fileName)) {
      recentlyViewed
          .removeWhere((element) => element.fileName == material.fileName);
    }
    recentBox.put(
        recentKey, recentlyViewed.map((e) => jsonEncode(e.toJson())).toList());
  }
